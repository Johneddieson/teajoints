...
export class AdmincheckoutPage implements OnInit {
      ...
      this.getCartDetails.map((i, index) => {
        i.Materials.map((iMat, index) => {
          if (i.Category == 'Milktea') {
            iMat.gramsperorder = i.ProductName.toLowerCase().includes('small')
              ? iMat.gramsperordersmall
              : iMat.gramsperordermedium;
          } else {
  ...
  dec(id, quantity) {
    for (let i = 0; i < this.getCartDetails.length; i++) {
      if (this.getCartDetails[i].id === id) {
        if (quantity != 1) this.getCartDetails[i].Quantity = quantity - 1;
        this.getCartDetails[i].Materials.map((materials, index) => {
          materials.Quantity = quantity - 1;
        });
      }
  ...
  singleDelete(data) {
    ...
    if (sessionStorage.getItem('cart')) {
      ...
      for (let i = 0; i < this.getCartDetails.length; i++) {
        if (
          this.getCartDetails[i].id === data.id &&
          this.getCartDetails[i].ProductName === data.ProductName
        ) {
  ...
  async OrderNow(paymentMethod: any) {
    if (
      ...
      if (this.total < 100 && paymentMethod.value == 'Online Payment') {
        this.minimumAlert();
      } 
      else {
  ...
  async OrderFunction(Name, Comments, paymentMethod) {
    var totalForPayment = parseFloat(this.total.toString()).toFixed(2);

    var totalamountReplace = totalForPayment.replace('.', '');
    var datetime = moment(new Date()).format('MM-DD-YYYY hh:mm A');

    if (paymentMethod == 'Online Payment') 
    {
      var descriptionForOnlinePayment = this.getCartDetails
        .map(function (e) {
          return `Orders : ${e.ProductName}(${e.Quantity} pcs), Unit price of ₱${e.UnitPrice} \n
              Fullname: ${Name} Walk-In`;
        })
        .join(', ');
      this.paymongoService
        .createPaymentLink(
          parseInt(totalamountReplace),
          descriptionForOnlinePayment,
          'Teajoints sizzling house'
        )
        .subscribe(
          (response) => {
            this.afstore
              ...
              .add({
                ...
                Comments: Comments,
                linkReference: response.data.attributes.checkout_url,
              })
              .then(async (el) => {
                //Decrease Function
                this.decreaseStock();
                  ...
                  .then(async (history) => {
                    ...
                    setTimeout(async () => {
                      ...
                      this.router.navigateByUrl(
                        `/invoicepage/${history.id}/history/POS`
                      );
  ...
  async decreaseStock() {
    //this.getMaterials()
    var getmaterial = this.getCartDetails.map(function (e) {
      return e.Materials;
    });
  ...
  condimentsQuantityValidation() {
    ...
    var mapMaterials = orders.map(function (e) {
      return e.Materials;
    });
  ...
  async minimumAlert() {
    var minimumAlert = await this.alertCtrl.create({
      message:
        'If your payment method is cash on delivery, your order must be greater than or equal to 100',
      buttons: [
        {
          text: 'Close',
          role: 'cancel',
